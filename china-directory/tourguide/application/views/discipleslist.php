...
?>
		...
		<?php  foreach(@$list_of_records as $records){  ?>
				<?php if($records->mentor_id == $userID){ ?> 
					...
					<tr>
					<th class="title">
						<img id="user-image-thumb<?php echo $records->id_no; ?>" src="<?php echo base_url() . "Welcome/userimage2/" . $records->id_no ?>" style="float:left; width:40px; height:40px; margin:0; margin-right:10px; padding:0; border-radius:20px; display:block; border:2px solid #2B86EC;">
						<a href="<?php echo base_url();?>Welcome/edit/<?php echo $records->id_no; ?>" style="margin-left:10px; margin-top:14px; font-size:12px; display:block;"><?php echo strtoupper($records->first_name. ' '. $records->maiden_name. ' '. $records->last_name); ?></a></th>
				

					<th class="date" style="text-align:center;"><a href="#"  data-toggle="modal" data-target="#myModal_<?php echo $records->id_no; ?>" style="text-align:center; font-size:13px; display:block;" > <?php echo disciplecount($list_of_records,$records->id_no);?></a>
					<!-- Trigger the modal with a button -->
								...
								<div class="modal-header" style="clear:both;">
								  ...
								  <div class="modal-body"> 
								   <ul id="list_of_data_records_list"> 
									<?php foreach(@$list_of_records as $child_of_this_disciple) { ?>
										<?php if($child_of_this_disciple->mentor_id == $records->id_no && $child_of_this_disciple->added_as_close_cell == 1 && $child_of_this_disciple->active == 0){ ?>
											...
											<li>
												...
												<a href="<?php echo base_url();?>/Welcome/edit/<?php echo $child_of_this_disciple->id_no;?>">
												<span style="clear:both; width:100px; display:block;"><?php echo strtoupper( $child_of_this_disciple->first_name .' '. $child_of_this_disciple->maiden_name .' '. $child_of_this_disciple->last_name );?></span></a>
											</li>
										<?php } ?>
								   <?php }?>
								   </ul>
						...
						<th style="text-align:center; font-size:12px;"><?php echo getRecordWithParentId($records->id_no, @$list_of_records);?></th>		
					 	...
					 	<!-- Modal -->
							...
							<div id="myModal_Edit<?php echo $records->id_no; ?>" class="modal fade deactivate_activate" role="dialog">
							  <div class="modal-dialog" style="width:60%;">
								...
								<div class="modal-header" style="clear:both; border-bottom:0;">
								...
								<script type="text/javascript">
								   ...
								   <form class='EditAccountInformation' style="">
								 
										  <div class="modal-body">
											<input type="hidden" id="delegatesID" name="delegatesID" value="<?php echo $records->id_no; ?>">
										   <div class="form-group">
												<a data-toggle="modal" data-target="#changeprofileuser_<?php echo $records->id_no;?>" style="position:relative; display:block; margin-bottom:20px;">
													<span style="background-image: url('<?php echo base_url(); ?>Welcome/images/loading2.gif'); background-position: center center; width:150px; height: 150px; background-repeat: no-repeat; font-size: 100px; display: block; margin:auto;">
														<img onload="this.style.opacity = 1" id="image_profile_<?php echo $records->id_no;?>" src="<?php echo base_url(); ?>Welcome/userimage2/<?php echo $records->id_no; ?>" style="opacity: 0.25; width:150px; margin-top: 0px; vertical-align: top; border:2px solid #2B86EC">
													</span>
												</a>
			
											<div style="float:left; width:45%; margin-bottom:20px;">
											
												<p style="text-align: left;margin:0; padding:0; color:#fff;">First Name</p>
												<input type="text" name="first_name" class="form-control" value="<?php echo $records->first_name; ?>">
												<p style="text-align:left; margin:0; padding:0; color:#fff;">Maiden Name</p>
												<input type="text" name="maiden_name" class="form-control" value="<?php echo $records->maiden_name; ?>">
												<p style="text-align:left; margin:0; padding:0; color:#fff;">Last Name</p>
										  ...
										  <div class="modal-footer" style="clear:both; border:0;">
